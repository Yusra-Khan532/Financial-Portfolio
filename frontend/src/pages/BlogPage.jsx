...
export default function BlogPage() {
  ...
  return <main className="pt-24 md:pt-28">
    ...
    <section className="px-6 py-8 md:px-10 md:py-10"><div className="mx-auto max-w-7xl">
      ...
      <div className="mt-6 border-b border-white/10 pb-6"><div className="flex max-w-full gap-2 overflow-x-auto pb-1" role="toolbar" aria-label="Research categories">{filters.map(({ value, label }) => <button key={value} onClick={() => setFilter(value)} className={`whitespace-nowrap border-b px-3 py-2 text-sm transition-colors focus:outline-none focus-visible:text-[#F5A623] ${filter === value ? "border-[#F5A623] text-[#F5A623]" : "border-transparent text-[#94A3B8] hover:border-white/25 hover:text-[#CBD5E1]"}`}>{label}</button>)}</div></div>
