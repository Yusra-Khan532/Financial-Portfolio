...
const suite = [["01","Research","A clearer frame for the questions worth asking."],["02","Portfolio Reviews","See how your investments work together."],["03","Financial Planning","Connect financial decisions to life goals."],["04","Wealth Planning","Evolve a long-term approach as capital grows."],["05","0 → 1 Investing","Build foundations before adding complexity."],["06","Global Investing","Understand where international diversification fits."],["07","Risk & Allocation","Treat risk and allocation as portfolio decisions."]];
...
export default function WhatWeDoPage(){const [cap,setCap]=useState(0);useEffect(()=>{const description="Discover how FinLitVentures helps investors move from financial confusion to structured investing through portfolio reviews, financial planning, wealth planning, global investing and research-led decision making.",old=document.title;document.title="Why We Exist | FinLitVentures";let meta=document.querySelector('meta[name="description"]');if(!meta){meta=document.createElement("meta");meta.setAttribute("name","description");document.head.appendChild(meta)}const previous=meta.getAttribute("content");meta.setAttribute("content",description);return()=>{document.title=old;meta.setAttribute("content",previous||"")}},[]);return <main className="pt-24 md:pt-28">
<section className="relative overflow-hidden border-b border-white/10 px-6 py-16 md:px-10 md:py-24"><div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_28%,rgba(45,212,191,.08),transparent_22%),radial-gradient(circle_at_65%_0%,rgba(212,175,55,.12),transparent_30%)]"/><div className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_.95fr] lg:gap-20"><Reveal><Label>Why We Exist</Label><h1 className="mt-5 max-w-xl font-serif-display text-5xl leading-[.94] tracking-tight text-white sm:text-6xl lg:text-7xl">Investing, <span className="text-[#E7C56B]">decoded.</span></h1><p className="mt-7 max-w-lg text-base leading-relaxed text-[#A6B4C4] md:text-lg">Research, planning and portfolio thinking, brought together around your goals, risk profile and capital — from your first rupee to long-term wealth.</p><div className="mt-9"><CTA to="/services">Explore Our Services</CTA></div></Reveal><Reveal delay={.15}><HeroGraphic/></Reveal></div></section>
